...
2026년 2월, Anthropic의 Claude Cowork 플러그인 11종 발표 이후 SaaS 기업 시가총액 &#36;2,850억이 증발했습니다. 이른바 **SaaSpocalypse**의 시작입니다.
...
CRM[^crm]에 로그인하고, 스프레드시트를 관리하고, 조달 워크플로우를 실행하는 것까지 — 에이전트 하나가 중간 관리직 10~15명의 행정 업무를 처리할 수 있다는 보고가 글로벌 금융/물류 대기업에서 나오고 있습니다.
...
총 증발 시가총액은 **약 &#36;2,850억~&#36;1조** 수준으로 추정됩니다(집계 범위에 따라 상이).
...
Gemini의 진짜 강점은 **멀티모달 통합**입니다.

- **100만 토큰 컨텍스트 윈도우**: 전체 코드베이스, 긴 문서를 한 번에 처리
- **네이티브 멀티모달**: 텍스트, 이미지, 오디오, 비디오를 동시에 이해하고 분석
...
학습과 연구 용도로는 Gemini가 가장 실용적인 선택입니다.
...
"AI를 처음 써보는 사람에게 뭘 추천하냐"는 질문에는 여전히 ChatGPT가 정답입니다.

### 기타 모델들

| 모델 | 소속 | 특징 |
|---|---|---|
| **Grok 4.1** (xAI) | Elon Musk의 xAI | Arena 스코어 1477로 2위권. 실시간 데이터 접근, "라이브"한 응답 |
...
[^crm]: **CRM(Customer Relationship Management)**은 고객 관계 관리 소프트웨어입니다. 고객 정보 통합 관리, 영업 파이프라인 추적, 마케팅 자동화, 고객 서비스 이력 관리 등이 주요 기능이며, 대표적인 도구로는 **Salesforce**(시장점유율 1위), **HubSpot**(중소기업용), **Microsoft Dynamics 365**(MS 생태계 연동), **Zoho CRM**(가성비) 등이 있습니다.
