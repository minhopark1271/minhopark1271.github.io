...
parent: 개발
nav_order: 29
description: "Claude Code 생산성 극대화 팁. Boris Cherny, Peter Steinberger, Andrej Karpathy의 실전 워크플로우와 Claude Code 팀의 실전 팁 10가지. 에이전트 코딩의 패러다임 전환과 선언적 프로그래밍 전략."
---

# Claude Code 사용팁
{:.no_toc}

Boris Cherny(Claude Code 제작자), Peter Steinberger(Moltbot 제작자), Andrej Karpathy(OpenAI 공동 창립자)의 AI 에이전트 활용 워크플로우와 Claude Code 팀의 실전 팁 10가지를 정리한 글입니다. 네 가지 소스의 실전 팁을 통해 생산성을 극대화하는 방법을 소개합니다.
...
# Part 4: Claude Code 팀의 실전 팁 10가지

Boris Cherny가 2026년 2월, Claude Code **팀 전체**의 사용법을 공개했습니다. Part 1이 Boris 개인의 워크플로우였다면, 이번에는 팀 내부에서 실제로 검증된 10가지 팁입니다. Boris는 "Claude Code를 사용하는 정답은 없다. 모든 사람의 환경이 다르니 직접 실험해보라"고 강조합니다.

---

## 병렬 작업이 최고의 생산성 팁

Claude Code 팀의 **1번 팁**은 병렬화입니다.

### 운영 방식

```
워크트리 A (za) ── Claude 세션 1: 기능 구현
워크트리 B (zb) ── Claude 세션 2: 버그 수정
워크트리 C (zc) ── Claude 세션 3: 리팩토링
워크트리 D (분석) ── Claude 세션 4: 로그 분석 + BigQuery (읽기 전용)
```
...
|------|------|
| 워크트리 수 | 3~5개 동시 운영 |
| 셸 별칭 | za, zb, zc로 한 키 전환 |
| 분석 전용 워크트리 | 코드 수정 없이 로그/메트릭만 분석 |
| 맥락 오염 방지 | 각 워크트리가 독립된 Claude 세션 유지 |

Part 1에서의 Boris 개인 방식(여러 탭)과 달리, 팀은 **git worktree + 셸 별칭** 조합으로 더 구조적인 병렬화를 달성합니다.

---

## 막히면 재계획 (Plan Mode 복구용)

Plan Mode는 단순히 작업 시작 전에만 쓰는 것이 아닙니다.
...
1. **Claude A**가 계획을 작성
2. **Claude B**가 "시니어 엔지니어" 역할로 그 계획을 검토
3. 검토 결과를 반영하여 계획 수정
4. 확정된 계획으로 구현 시작

Plan Mode는 **설정(setup)이 아니라 복구(recovery) 도구**로도 활용해야 합니다.

---

## Claude가 자신의 규칙을 작성하도록

CLAUDE.md를 Claude 스스로 개선하게 만드는 패턴입니다.

### 실천 방법

모든 수정 후 다음 프롬프트를 추가합니다:
...
- Claude가 실수할 때마다 **자체 규칙을 축적**
- 팀이 반복적으로 편집하여 실수율을 **측정 가능하게 감소**
- 일부 엔지니어는 작업별 노트 디렉토리를 만들고 CLAUDE.md에서 참조

Part 1에서 소개한 "CLAUDE.md 공유" 전략의 발전형으로, **Claude 자신이 규칙을 작성**한다는 점이 핵심입니다.

---

## 스킬을 기관 지식으로 전환

하루에 한 번 이상 반복하는 작업은 **무조건 스킬로 만듭니다**.

### 팀에서 실제 사용하는 스킬 예시

| 스킬 | 용도 |
|------|------|
| `/techdebt` | 세션 마지막에 중복 코드 탐지 및 제거 |
| Context dump | Slack, GDrive, Asana, GitHub 데이터를 통합 컨텍스트로 |
| Analytics agent | dbt 모델 작성, 코드 리뷰, 개발 환경 테스트 |
...
스킬을 git에 체크인
      ↓
새 팀원이 즉시 검증된 워크플로우 상속
      ↓
팀 전체의 생산성 누적 향상
```

스킬은 개인의 노하우가 아니라 **팀의 기관 지식**으로 전환됩니다.

---

## Claude에게 버그 수정 위임

Slack MCP를 연동하여 버그 수정을 자동화합니다.

### 워크플로우

```
Slack 버그 스레드 복사 → Claude에 붙여넣기 → "fix" 입력
      ↓
Claude가 자체적으로:
  - 코드 분석
  - 원인 파악
...
- **해결 방법이 아닌 문제 자체를 제시**
- Claude가 경로를 결정하도록 신뢰
- Docker 로그 분석, 분산 시스템 트러블슈팅도 자동 처리

이는 Part 3 Karpathy의 "선언적 접근"과 일맥상통합니다. 방법을 지시하지 말고 목표만 제시하는 것입니다.

---

## 도발적 프롬프팅

Claude를 단순한 어시스턴트가 아닌 **동료로 대하는** 세 가지 패턴입니다.

### 패턴 1: "Grill me on these changes"

Claude를 구현자가 아닌 **리뷰어**로 전환합니다. 자신이 작성한 코드에 대해 Claude가 날카롭게 질문하게 만듭니다.

### 패턴 2: "Prove to me this works"

단순 완료가 아니라 **증거를 요구**합니다. 메인 브랜치와 기능 브랜치의 동작 차이를 검증하게 합니다.

### 패턴 3: "Knowing everything you know now, scrap this"

현재까지 파악한 전체 맥락을 바탕으로 **처음부터 우아하게 다시 작성**하게 합니다. 점진적 수정보다 때로는 전면 재작성이 더 나은 결과를 냅니다.

이 접근법은 Part 2 Peter Steinberger의 "의도적으로 모호한 프롬프트"와 결합하면 더 강력해집니다.

---

## 터미널 환경 최적화

Claude Code 팀은 터미널 설정을 중요하게 여깁니다.

### 추천 환경

| 도구 | 용도 |
|------|------|
| **Ghostty** | 동기화된 렌더링, 24비트 색상, 유니코드 지원 |
| **/statusline** | 맥락 사용량 및 git 브랜치 실시간 표시 |
| 컬러 코딩 탭 | 워크트리별 시각 구분 |
| **음성 받아쓰기** | 타이핑 대비 3배 빠른 프롬프트 입력 |

### 음성 받아쓰기의 장점

```
타이핑: 짧고 축약된 프롬프트 → 모호함 증가
음성:   길고 상세한 프롬프트 → 정확도 향상 (3배 빠름)
```

음성 받아쓰기를 사용하면 프롬프트가 자연스럽게 길어지고, 그 결과 Claude의 이해도가 높아집니다.

---

## 서브에이전트로 맥락 정리

서브에이전트를 활용하여 메인 에이전트의 맥락 윈도우를 깨끗하게 유지합니다.

### 세 가지 패턴

```
패턴 1: "use subagents"
→ 추가 연산이 필요한 요청에 문구 추가

패턴 2: 오프로드
→ 분석/조사 작업을 서브에이전트에 위임
→ 메인 에이전트는 핵심 구현에 집중

패턴 3: 권한 라우팅
→ Opus 4.5로 권한 요청을 라우팅
→ 공격 스캔 및 자동 승인 처리
```

### 효과

- 메인 에이전트의 **맥락 윈도우 오염 방지**
- 여러 에이전트가 코드베이스의 다른 영역을 **동시 조사**
- Part 1의 서브에이전트 활용을 **더 구체적인 패턴**으로 발전

---

## Claude가 SQL을 대체

BigQuery 스킬을 코드베이스에 체크인하여 **팀 전원이 Claude 내에서 직접 분석**합니다.

### Boris의 증언

> "6개월 동안 SQL을 한 줄도 작성하지 않았습니다."

### 워크플로우

```
기존 방식:
  코드 작성 → 컨텍스트 전환 → SQL 도구 열기 → 쿼리 작성 → 결과 복사 → 돌아오기

팀의 방식:
  코드 작성 중 → Claude에게 데이터 분석 요청 → 즉시 결과 확인 → 계속 작업
```

**하나의 맥락에서 머무는 것**이 핵심입니다. Claude가 번역 계층(자연어 → SQL → 결과 해석)을 모두 처리합니다.

---

## Claude로 학습하기

생소한 코드베이스를 학습할 때 Claude를 **교사**로 활용합니다.

### 학습 도구 활용

| 방법 | 설명 |
|------|------|
| Explanatory 모드 | 변경의 이유를 설명하도록 활성화 |
| HTML 프레젠테이션 | 코드 설명을 시각적 슬라이드로 자동 생성 |
| ASCII 다이어그램 | 프로토콜 및 아키텍처 시각화 |
| 반복 학습 스킬 | 이해 설명 → Claude의 후속 질문 → 지식 격차 채우기 |

### 반복 학습 패턴

```
1. 특정 코드에 대해 이해한 내용 설명
      ↓
2. Claude가 후속 질문으로 지식 격차 탐지
      ↓
3. 부족한 부분을 채워가며 깊은 이해 도달
```

단순 질문-답변이 아니라 **쌍방향 학습 루프**를 만드는 것이 핵심입니다.

---

## 팀 워크플로우의 메타 패턴

10가지 팁은 다르지만, 기저에 통일된 철학이 있습니다.

| 철학 | 설명 |
|------|------|
| 병렬화 > 최적화 | 하나를 빠르게 하기보다 여러 개를 동시에 |
| Plan Mode는 복구용 | 막혔을 때 되돌아가는 도구 |
| Claude는 자기 개선 | 스스로 규칙을 쓰고 학습 |
| 스킬은 복합 성장 | 개인 노하우 → 팀 자산 |
| 지시가 아닌 도전 | Claude를 동료로 대하기 |
...
| 항목 | Boris (개인) | Peter | Karpathy | CC 팀 |
|------|-------------|-------|----------|-------|
| 병렬 운영 | 5~15개 | 5~10개 | 여러 CC 세션 | 3~5 워크트리 |
| 피드백 루프 | 품질 2~3배 | 폐쇄 루프 | 선언적 목표 | 자체 버그 수정 |
| Plan Mode | 대부분 Plan 시작 | - | - | 복구용으로도 활용 |
| CLAUDE.md | 팀 공유 | - | - | Claude가 자체 작성 |
...
| 초점 | 팀 협업 | 개인 생산성 | 패러다임 분석 | 실무 자동화 |
| 관점 | 실용적 팁 | 워크플로우 최적화 | 철학적 통찰 | 팀 스케일링 |

## 고유한 인사이트

| 소스 | 고유 기여 |
|------|----------|
| **Boris (개인)** | CLAUDE.md 팀 학습, 슬래시 커맨드, 훅 활용 |
| **Peter** | Prompt Request 개념, 프롬프트 리뷰 문화 |
| **Karpathy** | 선언적 vs 명령적, 능력 위축, Slopacolypse |
| **CC 팀** | 도발적 프롬프팅, 스킬의 기관 지식화, 음성 받아쓰기, SQL 대체 |
...
| 워크트리 병렬화 | 3~5개 워크트리 + 셸 별칭으로 운영 | CC 팀 |
| 재계획 습관 | 막히면 Plan Mode로 복귀 | CC 팀 |
| 자체 규칙 작성 | Claude가 CLAUDE.md 직접 업데이트 | CC 팀 |
| 스킬 기관화 | 반복 작업을 스킬로 만들어 git 체크인 | CC 팀 |
| 도발적 프롬프팅 | Grill me / Prove it / Scrap this | CC 팀 |
| 음성 받아쓰기 | 타이핑 대비 3배 빠른 상세 프롬프트 | CC 팀 |
| SQL 대체 | BigQuery 스킬로 6개월간 SQL 미작성 | CC 팀 |
| 학습 도구 | Explanatory 모드, 반복 학습 루프 | CC 팀 |

---

# 결론

네 소스의 공통된 메시지는 명확합니다:
...
5. **완벽주의를 버리고** 결과에 집중하며
6. **스킬과 CLAUDE.md를 팀 자산으로** 축적하고
7. Claude를 어시스턴트가 아닌 **동료로 도전적으로** 대하는 것

Claude Code와 AI 에이전트를 단순한 코딩 도우미가 아닌, **개발 워크플로우의 핵심 인프라**로 활용하는 것이 네 소스의 공통된 철학입니다.
